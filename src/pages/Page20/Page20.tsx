...
export const Page20 = () => {
  ...
  const Comment = (
    <WithModal text="бесконечный поток информации" triggerType="author" mode="text">
      Так долго пытался адекватно реализовать этот эффект,
      что ради него сделал целую отдельную главу в книге.
      Но ведь круто же!
    </WithModal>
  );
  ...
  return (
    <PageWrapper>
      <P>
        Мониторы отображали {Comment} об уровне излучений, состоянии корабля и члена экипажа.
      </P>

      <P>
        Как обычно — мелькало предупреждение об учащённом сердцебиении
        и рекомендация о приёме успокоительного.
      </P>

      <P>
        Егор хотел минимизировать свою зависимость от чего бы то ни было.
        Он и так, мол, зависит от кислорода, воды, витаминов и аминокислот,
        так ещё и на успокоительное подсесть ему предлагают.
        Вот уж дудки! Поэтому без крайней необходимости он не употреблял подобные препараты.
        А уж тем более алкоголь и наркотики.
        Он так боялся зависимостей, что это вполне можно было считать за настоящую фобию.
        С таким прошлым &mdash; немудрено.
      </P>
    </PageWrapper>
  );
};
